...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!id) return res.status(400).json({ message: "Param id is mandatory." });

    const query = categoriesQueries.findById(id);
    const category = await db.query(query);

    if (!category.rows[0]) return res.status(404).json({ message: "Cartegory not found." });
    ...
    if (!deleteResponse.rows[0]) return res.status(404).json({ message: "Cartegory not deleted." });
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (!id) return res.status(404).json({ message: "Param id is mandatory." });

    if (name.length < 3)
      return res.status(404).json({ message: "Name must be at least 3 characters" });

    const query = categoriesQueries.findById(id);
    const category = await db.query(query);
    if (!category.rows[0]) return res.status(404).json({ message: "Caegory not found." });
    ...
    if (!updateResponse.rows[0])
      return res.status(404).json({ message: "Categories not updated." });
